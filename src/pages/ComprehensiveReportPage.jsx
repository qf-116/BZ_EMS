...
export default function ComprehensiveReportPage() {
  ...
  const filteredRows = useMemo(() => rows.filter(r => {
    ...
    return true;
  }), [rows, device, workshop, workshopOf]);
  ...
  return (
    <>
      <PageHeader
        title="设备综合运行分析报表"
        subtitle={`状态 · 稼动率 · OEE · 报警 · 质量 · 产出综合视图 · 演示日 ${meta.demoDay || '--'} · 口径截止 ${meta.lastSampleAt || '--'}`}
      ...
      <Alert
        ...
        showIcon
        style={{ marginBottom: 12 }}
        message={`口径：OEE / 可用率 / 性能率 / 质量率为演示日 ${meta.demoDay || '--'} 的按天窗口结果（由生产输入 + 停机事实 + 速度配置推导，不可计算时显示 '--' 并悬浮说明原因）；运行状态 / 稼动率 / 报警 / 数据完整率 / 参数不一致数来自报表快照汇总。两路数据按设备合并，缺失列显示 '--'。`}
      />
      {filteredRows.length === 0 ? (
        <EmptyState
          description="当前筛选条件下无综合分析行"
          reason="演示快照中该设备既无按天生产数据，也无综合报表汇总行，调整筛选或重置后重试"
          next="重置筛选"
          nextLabel="重置筛选"
          onNext={handleReset}
        />
