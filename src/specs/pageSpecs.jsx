...
const DATA_SOURCE_NOTE = (
  <>
    <p>本系统为演示数据驱动模式：所有页面数据来自 DemoStore 共享状态层，持久化于 localStorage 键 <code>dms-demo:state</code>（快照结构版本 STORE_VERSION = 3，版本不符或解析失败自动恢复初始剧本并提示）。工作台提供「重置演示」一键恢复三个标准演示剧本的初始状态。</p>
  ...
  '/binding-overview': {
    ...
    content: (
      <>
        ...
        <Fields items={[
          ['校验规则（保存时全部通过）', '恰好 1 个启用主数据源（role = main）；启用项 ≥ 1 且至少勾选一项有效（非失效）指标；IoT 来源编码全局唯一（不与其他设备活动绑定冲突、同绑定内不重复）；失效指标（如 M.ambient_humidity）不能参与绑定'],
    ...
    content: (
      <>
        ...
        <ol>
          ...
          <li>新增 / 编辑档案为演示只读口径之外的宿主能力：本页不提供资产字段编辑（跨系统主数据由宿主维护）。</li>
        </ol>
        <h4>边界与显示规则</h4>
        <ul>
    ...
    content: (
      <>
        ...
        <Fields items={[
          ...
          ['绑定实例（只读展示）', '1 主 + N 子传感器结构：主设备承载状态类指标，子传感器承载温度 / 振动等专用指标；展示来源编码（IOT-D-* / IOT-S-*）与角色'],
...
const inspectionContent = (
  <>
    ...
    <h4>交互规则与边界</h4>
    <ol>
      <li>列表页：编号 / 名称 / 类型 / 状态筛选；项目与标准的新增 / 编辑为弹窗表单（演示不落库，删除仅提示由点巡保养业务模块执行）。</li>
...
const maintenanceContent = (
  <>
    ...
    <h4>交互规则与边界</h4>
    <ol>
      <li>列表页：关键词 / 分组 / 状态筛选；项目与标准的新增 / 编辑为弹窗（演示不落库）。</li>
...
const patrolContent = (
  <>
    ...
    <h4>交互规则与边界</h4>
    <ol>
      <li>列表页：关键词 / 状态筛选；项目与标准新增 / 编辑为弹窗（演示不落库）。</li>
...
export function specForPath(path) {
  ...
  // 范围外路由（§0.3 / §7.3）：系统管理（权限 / 审计）由宿主平台提供，统一范围外口径
  if (path === '/permissions' || path === '/audit') return outOfScopeSpec;
